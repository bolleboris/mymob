...
$BMUCore->ProviderSA(MYMOB_APP_ID)->Offer()->GetList();
$rs = $BMUCore->sendRequest();


foreach ($rs['result']['offers'] as $offer) {
  /* $person_attributes = get_person_attributes($offer['customer_id']);
   if ($person_attributes == false) {
	  $person_attributes = array('Voornaam' => $offer['customer_code']);
   }*/
   $rows[] = array(
	   "OfferNr" => $offer['offer_id'],
	   "VerschafferNr" => $offer['supplier_id'],
	   "Verschaffer" => $offer['supplier_code'],
       	   "LeverancierNr" => $offer['provider_id'],
       	   "Leverancier" => $offer['provider_code'],
       	   "OfferSoortNr" => $offer['offer_type_id'],
       	   "OfferSoort" => $offer['offer_type_code'],
       	   "DatumAangemaakt" => $offer['create_dt'],
       	   "Status" => $offer['status'],
       	   "DatumAangevraagd" => $offer['request_dt'],
       	   "GeldigVan" => $offer['valid_from'],
       	   "GeldigTot" => $offer['valid_to']
   );
   $i++;
...
$metaData = array(
	...
	"fields" => array(
		array('name' => 'OfferNr'),
		array('name' => 'VerschafferNr',),
		array('name' => 'Verschaffer',),
		array('name' => 'LeverancierNr',),
		array('name' => 'Leverancier',),
		array('name' => 'OfferSoortNr'),
		array('name' => 'OfferSoort'),
		array('name' => 'DatumAangevraagd'),
		array('name' => 'DatumAangemaakt'),
		array('name' => 'Status'),
		array('name' => 'GeldigVan'),
		array('name' => 'GeldigTot'),
	)
);
...
$EmptyRows[] = array(
	...
	"OfferSoort" => null,
	"OfferSoortNr" => null,
	"DatumAangevraagd" => null,
	"DatumAangemaakt" => null,
	"Status" => null,
	"GeldigVan" => null,
	"GeldigTot" => null,
);
